...
이 저장소는 원본 `revfactory1/harness` 저장소를 Hermes Agent용으로 포팅한 프로젝트입니다.
즉, `revfactory1/harness`의 핵심 개념을 Hermes에 맞게 다시 구성한 것이며, 도메인 분석, 멀티에이전트 위임 패턴 설계, 재사용 가능한 스킬 생성, 그리고 Hermes 도구 체계에 맞는 오케스트레이션 문서 작성을 목표로 합니다.
